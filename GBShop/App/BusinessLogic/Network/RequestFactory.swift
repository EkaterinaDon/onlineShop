...
class RequestFactory {
    ...
    func makeChangeDataRequestFactory() -> ChangeDataRequestFactory {
        let errorParser = makeErrorParser()
        return ChangeData(errorParser: errorParser,
                          sessionManager: commonSession,
                          queue: sessionQueue,
                          baseUrl: baseUrl)
    }
    ...
    func makeAddToBasketRequestFactory() -> AddToBasketRequestFactory {
        let errorParser = makeErrorParser()
        return AddToBasketRequest(errorParser: errorParser,
                                  sessionManager: commonSession,
                                  baseUrl: baseUrl)
    }
    ...
    func makeRemoveFromBasketRequestFactory() -> RemoveFromBasketRequestFactory {
        let errorParser = makeErrorParser()
        return RemoveFromBasketRequest(errorParser: errorParser,
                                       sessionManager: commonSession,
                                       baseUrl: baseUrl)
    }
    ...
    func makePayBasketRequestFactory() -> PayBasketRequestFactory {
        let errorParser = makeErrorParser()
        return PayBasketRequest(errorParser: errorParser,
                                sessionManager: commonSession,
                                baseUrl: baseUrl)
    }
}
